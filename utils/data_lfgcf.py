...
class DataUtils():
    ...
    def build_graph(self, train_data, user2tag, item2tag, data_stat):
        ...
        row, col = user2tag[:, 0], user2tag[:,1] + n_users
        row, col = np.concatenate([row, col]), np.concatenate([col, row])
        idx = np.unique(np.stack([row, col]), axis=1)
        vals = [1.] * (idx.shape[1])
        user_adj = sp.coo_matrix((vals, idx), shape=(n_users + n_tags, n_users + n_tags))

        row, col = item2tag[:, 0], item2tag[:,1] + n_items
        row, col = np.concatenate([row, col]), np.concatenate([col, row])
        idx = np.unique(np.stack([row, col]), axis=1)
        vals = [1.] * (idx.shape[1])
        item_adj = sp.coo_matrix((vals, idx), shape=(n_items + n_tags, n_tags + n_items))

        row, col = np.concatenate([user2tag[:,1], item2tag[:,1]]), np.concatenate([user2tag[:,0] + n_tags, item2tag[:,0] + n_users + n_tags])
        row, col = np.concatenate([row, col]), np.concatenate([col, row])
        idx = np.unique(np.stack([row, col]), axis=1)
        vals = [1.] * (idx.shape[1])
        tag_adj = sp.coo_matrix((vals, idx), shape=(n_tags + n_users + n_items, n_tags + n_users + n_items))
        return _bi_norm_lap(user_adj).tocsr()[:n_users, n_users:].tocoo(), _bi_norm_lap(item_adj).tocsr()[:n_items, n_items:].tocoo(), _bi_norm_lap(tag_adj).tocsr()[:n_tags, n_tags:].tocoo()
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return user, pos_item, neg_item, tag
